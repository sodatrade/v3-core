...
import { TestSodaTradeV3SwapPay } from '../typechain/TestSodaTradeV3SwapPay'
...
import { TestSodaTradeV3ReentrantCallee } from '../typechain/TestSodaTradeV3ReentrantCallee'
...
describe('SodaTradeV3Pool', () => {
  ...
  let factory: SodaTradeV3Factory
  let pool: MockTimeSodaTradeV3Pool

  let swapTarget: TestSodaTradeV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeSodaTradeV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestSodaTradeV3ReentrantCallee')
      ).deploy()) as TestSodaTradeV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestSodaTradeV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestSodaTradeV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestSodaTradeV3SwapPay
